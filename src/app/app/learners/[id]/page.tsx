...
export default function LearnerDetailsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function handleSignDocument(docId: string) {
        if (!learner) return;
        if (!confirm("Lancer la procédure de signature électronique pour ce document ?")) return;

        const res = await initiateDocumentSignatureAction(
            docId,
            learner.email,
            learner.firstName,
            learner.lastName,
        ...
        if (res.success) {
            toast({ title: "Signature demandée", description: "L'email a été envoyé au signataire." });
            loadData();
        } else {
            toast({ title: "Erreur", description: res.error, variant: "destructive" });
        }
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20">
            ...
            <div className="p-8 space-y-6 max-w-7xl mx-auto">
                ...
                <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-3">
                        {!learner.userId ? (
                            <Button
                                variant="outline"
                                className="border-indigo-200 text-indigo-700 bg-indigo-50 hover:bg-indigo-100 gap-2"
                                onClick={async () => {
                                    if (!confirm(`Créer un compte accès pour ${learner.email} ?`)) return;
                                    const { grantLearnerAccessAction } = await import('@/application/actions/learner.actions');
                                    const res = await grantLearnerAccessAction(learner.id, learner.email);
                                    if (res.success && res.data) {
                                        toast({
                                            title: "Accès créé !",
                                            description: `Mot de passe temporaire : ${res.data.tempPassword}`,
                                            duration: 10000,
                                        });
                                        loadData();
                                    } else {
                                                ...
                                                <div className="flex items-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                                    {doc.fileUrl ? (
                                                        <>
                                                            ...
                                                            {/* YOUSIGN INTEGRATION */}
                                                            {(doc.type === 'CONVENTION_FORMATION' || doc.type === 'CONTRAT_FORMATION') && !doc.yousignProcedureId && (
                                                                <Button
                                                                    size="sm"
                                                                    className="h-8 gap-2 bg-teal-600 hover:bg-teal-700 text-white border-teal-600"
                                                            ...
                                                            {doc.yousignProcedureId && (
                                                                <Badge variant="outline" className="h-8 bg-teal-50 text-teal-700 border-teal-200 gap-1 px-2">
                                                                    <PenTool size={12} /> {doc.yousignStatus || 'En cours'}
                                                                </Badge>
                                                            )}

                                                            {doc.status === 'PENDING_REVIEW' && !doc.yousignProcedureId && (
                ...
                />
                {/* Communication Modal */}
                {isWhatsAppOpen && learner && (
                    <CommunicationModal
                        leadIds={[learner.id]}
                        onClose={() => setIsWhatsAppOpen(false)}
                        onSuccess={() => setIsWhatsAppOpen(false)}
                        targetType="learner"
                        title={`Message à ${learner.firstName}`}
                    />
                )}
            </div >
        </div >
